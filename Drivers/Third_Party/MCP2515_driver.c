 ...
 *      Author: matt
 *  Edited on: 5 Jun 2026
 *      Editor: whp27
 */

#include "MCP2515_driver_const.h"
...
CAN_Error MCP_readMessageFrom(RXBn rxbn, can_frame *frame) {
	...
	uint32_t id = (rxReg.rx_reg_array[MCP_SIDH] << 3)
			+ (rxReg.rx_reg_array[MCP_SIDL] >> 5);
...
void MCP_clearTXInterrupts(void) {
	modifyRegister(MCP_CANINTF, (CANINTF_TX0IF | CANINTF_TX1IF | CANINTF_TX2IF),
			0);
}
