...
#include "CAN_TxTask.h"
#include "ControlTask.h"
#include "SafetyWatchdogTask.h"
...
const osThreadAttr_t commsTask_attributes = { .name = "commsTask", .stack_size =
...
// Concrete allocation of our inter-task communication lines
QueueHandle_t NavigationQueue = NULL;
QueueHandle_t TelemetryQueue = NULL;

// Global atomic states
volatile uint8_t system_errors = FAULT_NONE;
volatile RobotState_t current_robot_state = STATE_INITIALIZING;
...
int main(void) {
	...
	// Create the Navigation Queue (Holds 5 navigation command structures max)
	  NavigationQueue = xQueueCreate(5, sizeof(NavCommand_t));
	  if (NavigationQueue == NULL) {
	      // Hardware initialisation fault handler
	      Error_Handler();
	  }

	  // Create the Telemetry Queue (Holds 5 telemetry payload updates max)
	  TelemetryQueue = xQueueCreate(5, sizeof(TelemetryData_t));
	...
	safetyTaskHandle = osThreadNew(StartSafetyTask, NULL,
...
static void MX_GPIO_Init(void) {
	...
	HAL_GPIO_WritePin(GPIOC,
			MOTOR_L_IN1_Pin | MOTOR_L_IN2_Pin | MOTOR_R_IN1_Pin
					| MOTOR_R_IN2_Pin | MOTOR_STBY_Pin, GPIO_PIN_RESET);

	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOA, CAN_CS_Pin | US_FRONT_TRIG_Pin | US_REAR_TRIG_Pin,
			GPIO_PIN_RESET);

	/*Configure GPIO pins : MOTOR_L_IN1_Pin MOTOR_L_IN2_Pin MOTOR_R_IN1_Pin MOTOR_R_IN2_Pin
	 MOTOR_STBY_Pin */
	GPIO_InitStruct.Pin = MOTOR_L_IN1_Pin | MOTOR_L_IN2_Pin | MOTOR_R_IN1_Pin
			| MOTOR_R_IN2_Pin | MOTOR_STBY_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);

	/*Configure GPIO pins : CAN_CS_Pin US_FRONT_TRIG_Pin US_REAR_TRIG_Pin */
	GPIO_InitStruct.Pin = CAN_CS_Pin | US_FRONT_TRIG_Pin | US_REAR_TRIG_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
void StartControlTask(void *argument) {
	...
	// Hand over execution to your completely isolated C file
	controlTask();
